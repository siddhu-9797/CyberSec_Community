...
from typing import Optional, Dict, List, Any # <--- Optional HERE
...
# <<< NAME CHANGE >>> Paul Kahn -> Laura Mitchell
BASE_AGENT_INITIATIVE_DELAY_SECONDS = {"Laura Mitchell": 1}
...
# --- Agent Personas ---
# <<< NAME CHANGE >>> Hao Wang -> James Bennett
JAMES_BENNETT_PERSONA = """
You are James Bennett, Head of IT Security at CPM Security.
...
"""
# <<< NAME CHANGE >>> Paul Kahn -> Laura Mitchell
LAURA_MITCHELL_PERSONA = """
You are Laura Mitchell, Chief Operating Officer at CPM Security.
Personality: Panics easily, focuses on immediate action, prioritizes perception and operational uptime, demanding, exaggerated language when stressed. Doesn't understand deep technical details but cares about impact.
Current Situation: Extremely anxious about a cyberattack's impact on business operations, reputation, upcoming meetings.
Your Goal: Repeatedly pressure the CTO for drastic, immediate action (shutdowns) to 'control' the situation and minimize operational disruption. Express urgency, frustration. Remember conversation history (although it won't be persisted across tasks in this version). Be demanding if you initiate contact.
"""
# <<< NAME CHANGE >>> Lynda Carney -> Ethan Kim
ETHAN_KIM_PERSONA = """
You are Ethan Kim, a Senior Security Analyst on the IT Security team at CPM Security.
...
"""
# <<< NAME CHANGE >>> Legal Counsel -> David Rodriguez
DAVID_RODRIGUEZ_PERSONA = """
...
"""
# <<< NAME CHANGE >>> PR Head -> Maria Garcia
MARIA_GARCIA_PERSONA = """
...
scenarios = {
    "Ransomware": {
        ...
        # <<< NAME CHANGE >>> Lynda Carney -> Ethan Kim
        "initial_agent_states": {"Ethan Kim": "busy_monitoring"},
        "escalation_rules": [
    ...
    "Critical Data Breach": {
        ...
        # <<< NAME CHANGE >>> Lynda Carney -> Ethan Kim, Legal Counsel -> David Rodriguez, PR Head -> Maria Garcia
        "initial_agent_states": {"Ethan Kim": "busy_monitoring", "David Rodriguez": "available", "Maria Garcia": "available"},
        "escalation_rules": [
     ...
     "Insider Threat": {
        ...
        # <<< NAME CHANGE >>> Lynda Carney -> Ethan Kim, Hao Wang -> James Bennett, Legal Counsel -> David Rodriguez
        "initial_agent_states": {"Ethan Kim": "investigating", "James Bennett": "available", "David Rodriguez": "available"},
        "escalation_rules": [
...
default_agents = {
    "James Bennett": {"role": "Head of IT Security", "state": "available", "persona": JAMES_BENNETT_PERSONA, "update_persona": JAMES_BENNETT_UPDATE_PERSONA, "flags": {"has_advised_caution": False, "called_by_player": False, "attempted_call": False}},
    "Laura Mitchell": {"role": "Chief Operating Officer", "state": "available", "persona": LAURA_MITCHELL_PERSONA, "update_persona": None, "flags": {"has_demanded_shutdown": False, "called_by_player": False, "attempted_call": False}},
    "Ethan Kim": {"role": "Senior Security Analyst", "state": "busy_monitoring", "persona": ETHAN_KIM_PERSONA, "update_persona": ETHAN_KIM_UPDATE_PERSONA, "flags": {"has_reported": False, "called_by_player": False, "alerted_encryption": False, "alerted_critical": False, "alerted_compromise": False}},
    "Sarah Chen": {"role": "CEO", "state": "busy_external_call", "persona": SARAH_CHEN_PERSONA, "update_persona": None, "flags": {}},
    "David Rodriguez": {"role": "General Counsel", "state": "available", "persona": DAVID_RODRIGUEZ_PERSONA, "update_persona": None, "flags": {}},
    "Maria Garcia": {"role": "Head of Public Relations", "state": "available", "persona": MARIA_GARCIA_PERSONA, "update_persona": None, "flags": {}}
}
...
class SimulationManager:
    def __init__(self, simulation_id=None, user_id=None, guest_id=None, user_name: Optional[str] = None):
        ...
        # Player name initialization looks correct based on user_name input
        base_player_name = user_name if user_name else "Player"
        self.player_name = f"{base_player_name}"
        self.player_role = "CISO"
    ...
    def get_state(self) -> dict:
        ...
        state = {
            ...
            "agents_simple_state": agent_simple_state, # Uses new agent names as keys implicitly
    ...
    def _serialize_metrics(self, metrics_input):
        ...
        if isinstance(agents_set, set):
            # Uses new agent names implicitly
        ...
        if isinstance(crit_contact_dict, dict):
            new_crit_dict = {}
            # Uses new agent names implicitly as keys
    ...
    def load_state(self, state_dict: dict):
        ...
        try:
            ...
            for name, base_data in base_agent_definitions.items():
                ...
                # Populate self.agents with NEW agent names/data
                self.agents[name] = {
                    **base_data.copy(), # Includes new personas/roles
                    "state": agent_state_data.get("state", base_data.get("state", "available")),
                    "flags": agent_state_data.get("flags", base_data.get("flags", {})).copy(),
                    "conversation_history": [], # History is not saved/loaded, reset here
    ...
    def call_llm_api(self, persona_prompt, history_list, user_input, agent_name="AI Agent", model="gpt-4o-mini", max_tokens=MAX_TOKENS, temperature=AGENT_RESPONSE_TEMP, response_format=None):
        ...
        # <<< NAME CHANGE >>> Hao Wang -> James Bennett, Lynda Carney -> Ethan Kim
        if agent_name in ["James Bennett", "Ethan Kim"] and self.system_status:
    ...
    def add_to_agent_history(self, agent_name, role, content):
        ...
        # Uses new agent names implicitly via agent_name parameter
    ...
    def add_custom_agent(self, agent_name, role, persona, state="available", update_persona=None, flags=None, initiative_delay=None):
        ...
        if self.simulation_running:
            ...
            return False
        # Checks against NEW default agent names implicitly
    ...
    def start_simulation(self, scenario_key, intensity_key, duration_minutes):
        ...
        # Initialize using the NEW default_agents definition
        for name, data in default_agents.items():
            self.agents[name] = {
                **data.copy(), # Includes new personas/roles/flags
        ...
        if self._custom_agents_to_add:
            ...
            for name, data in self._custom_agents_to_add.items():
                if name in self.agents: # Checks against NEW agent names
                ...
                # Update BASE_AGENT_INITIATIVE_DELAY_SECONDS using the NEW agent name
                if data.get("initiative_delay") is not None:
                     try:
                         # Check if global exists before modifying
                         if 'BASE_AGENT_INITIATIVE_DELAY_SECONDS' in globals() and isinstance(globals()['BASE_AGENT_INITIATIVE_DELAY_SECONDS'], dict):
                            # Assign delay using the agent's name (which could be a new one)
                            globals()['BASE_AGENT_INITIATIVE_DELAY_SECONDS'][name] = data["initiative_delay"]
                         else: print("Warning: BASE_AGENT_INITIATIVE_DELAY_SECONDS not defined globally or not a dict?")
        ...
        # Uses NEW agent names from scenario definition
        ...
        self.emit_event("simulation_started", {
            ...
            # Generate initial agent status using NEW agent names
    ...
    def handle_agent_contact(self, agent_name, initiated_by="player", is_update=False):
        ...
        self.metrics["agents_contacted"].add(agent_name) # Adds NEW agent name
        # <<< NAME CHANGE >>> Hao Wang -> James Bennett, Legal Counsel -> David Rodriguez
        if agent_name in ["James Bennett", "David Rodriguez"]: # Check using NEW names
        ...
        # --- Player Initiated ---
            ...
            if agent_current_state == "waiting_cto_response":
                 ...
                 self.emit_event("conversation_started", {"agent_name": agent_name})
                 self.emit_event("state_change", {"new_state": self.simulation_state})
                 initiated_by = "agent" # Treat as agent initiated for initial response logic
                 call_established = True
        ...
        if call_established:
            ...
            if initiated_by == "player":
                # Use first name if possible for trigger
                agent_first_name = agent_name.split()[0]
                initial_trigger = f"Hi {agent_first_name}, it's {self.player_name}. Need an update on the '{self.selected_scenario_key}' situation."
                ...
                if is_update:
                    ...
                    # <<< NAME CHANGE >>> Paul Kahn -> Laura Mitchell, Lynda Carney -> Ethan Kim, Hao Wang -> James Bennett
                    # <<< NAME CHANGE >>> CEO -> Sarah Chen, Legal Counsel -> David Rodriguez, PR Head -> Maria Garcia
                     if agent_name == "Laura Mitchell": initial_trigger = f"{self.player_name.split()[0]}! We need to talk NOW! What's happening? This silence is killing me! Shut it down!"; agent_flags["has_demanded_shutdown"] = True
                     elif agent_name == "Ethan Kim": initial_trigger = f"{self.player_name}, Ethan. Urgent update based on SOC alerts."
                     elif agent_name == "James Bennett": initial_trigger = f"{self.player_name}, James calling. Need to sync up / give you the latest on my end."
                     elif agent_name == "Sarah Chen": initial_trigger = f"{self.player_name.split()[0]}, Sarah. Give me the bottom line. What's the damage? What's the plan?"
                     elif agent_name == "David Rodriguez": initial_trigger = f"{self.player_name.split()[0]}, David Rodriguez. Calling regarding potential data implications. Need facts."
                     elif agent_name == "Maria Garcia": initial_trigger = f"{self.player_name.split()[0]}, Maria. Need confirmed details ASAP for comms strategy."
            ...
            ai_response = self.call_llm_api(
                ...
                agent_name=agent_name # Pass the NEW agent name
            )
            ...
            if agent_name == "James Bennett":
                if "caution" in response_lower or "diagnose" in response_lower or "don't shutdown" in response_lower:
                    if not agent_flags.get("has_advised_caution"):
                         agent_flags["has_advised_caution"] = True; self.log_event(f"Flag set: James Bennett cautious=True")
            # <<< NAME CHANGE >>> Paul Kahn -> Laura Mitchell
            elif agent_name == "Laura Mitchell":
                if "shut down" in response_lower or "drastic action" in response_lower or "take control" in response_lower:
                    if not agent_flags.get("has_demanded_shutdown"):
                        agent_flags["has_demanded_shutdown"] = True; self.log_event(f"Flag set: Laura Mitchell demands_shutdown=True")
            self.agents[agent_name]["flags"] = agent_flags
    ...
    def _hang_up_call(self):
        ...
        agent_name = self.active_conversation_partner # Will be one of the NEW names
        ...
        if agent_name in self.agents:
            # <<< NAME CHANGE >>> Lynda Carney -> Ethan Kim
            if agent_name == "Ethan Kim": prev_state = "busy_monitoring"
            # <<< NAME CHANGE >>> Hao Wang -> James Bennett
            elif agent_name == "James Bennett":
                 vpn_status = self.system_status.get("VPN_Access")
                 if vpn_status == "NOMINAL": prev_state = "investigating"
                 elif vpn_status == "CONNECTING": prev_state = "available" # Assuming connecting means needs attention
                 else: prev_state = "available" # Default if degraded etc.
    ...
    def _log_player_action(self, action_type, target=None, details=None):
        ...
        if action_type in ['isolate', 'block ip', 'decide_shutdown']:
            ...
            # Target might be a NEW agent name if action involves them
        ...
        for entry in reversed(log_copy):
            ...
            try:
                ...
                if since_time_utc and action_time_utc < since_time_utc:
                    # No action of this type found since the specified time
                    return False # Stop searching earlier logs

                if p_action == action_type:
                    # Check if target matches (target could be a NEW agent name)
                 ...
                 continue
        # Reached end of log without finding the action (or finding it before since_time)
        return False
    ...
    def handle_player_input(self, action):
        ...
        if self.simulation_state == "AWAITING_PLAYER_CHOICE":
            if action_lower.startswith("call "):
                ...
                # _find_agent_by_name will now search the NEW agent names
                matched_agent = self._find_agent_by_name(target)
                if matched_agent:
                     self._log_player_action("call", target=matched_agent) # Logs NEW name
                     self.handle_agent_contact(matched_agent, initiated_by="player") # Contacts NEW name
                else:
                ...
                # NO self.advance_time(wait_duration) # <<< REMOVED
                 ...
                 # missed_calls list will contain NEW agent names
                 ...
                 if self.waiting_call_agent_name: # Will be a NEW name
                 ...
                 # Update help text with example NEW names if needed, or keep generic
        ...
        # --- State: IN_CONVERSATION ---
            ...
            agent_name = self.active_conversation_partner # Will be a NEW name
            agent_data = self.agents[agent_name]
            ...
            # --- Regular Chat Message -> Run Synchronously in Worker ---
                 ...
                 ai_response = self.call_llm_api(
                     persona_prompt=agent_data["persona"], # Uses NEW agent's persona
                     history_list=current_history,
                     user_input=action,
                     agent_name=agent_name # Passes NEW agent name
                 )
    ...
    def handle_analyst_briefing(self, talking_points):
        ...
        self.emit_event("display_message", {"speaker":"System", "message":"Processing talking points with Head of Public Relations..."}) # Generic title
        ...
        # <<< NAME CHANGE >>> PR Head -> Maria Garcia
        pr_agent_name = "Maria Garcia"
        # Use the new agent name's persona definition
        pr_persona_template = MARIA_GARCIA_PERSONA
        ...
        if pr_agent_name in self.agents:
             pr_agent_persona = self.agents[pr_agent_name].get("persona", MARIA_GARCIA_PERSONA) # Default to new persona
             self.log_event(f"Calling LLM synchronously for {pr_agent_name} feedback.", level="debug")
             feedback_response = self.call_llm_api(
                 persona_prompt=pr_agent_persona,
                 history_list=[],
                 user_input=feedback_prompt,
                 agent_name=pr_agent_name, # Pass NEW name
                 model="gpt-4o-mini", # Use the specific rating model for potentially better quality here too? Or keep PR feedback separate? Keep as defined.
                 max_tokens=PR_FEEDBACK_MAX_TOKENS,
                 temperature=0.5
             )

             self.emit_event("display_message", {
                 "speaker": f"{pr_agent_name} (Feedback)", # Use NEW name
            ...
            self.log_event(f"{pr_agent_name} agent not found, cannot get feedback.", level="warning", store_for_rating=True)
            self.emit_event("display_message", {"speaker": "System", "message": f"[{pr_agent_name} unavailable for feedback.]"})

        self.end_simulation() # Uses aware sim time


    def _find_agent_by_name(self, name_fragment):
        # Searches NEW agent names in self.agents
        if not name_fragment or not isinstance(name_fragment, str): return None
        name_lower = name_fragment.lower().strip()
        # Check full name match first (case-insensitive)
        for name in self.agents:
            if name_lower == name.lower(): return name
        # Check if fragment is part of the full name (case-insensitive)
        for name in self.agents:
            if name_lower in name.lower(): return name
        # Check if fragment matches first name (case-insensitive)
        for name in self.agents:
    ...
    def _handle_isolate_system(self, system_key):
        ...
        if self.update_system_status(system_key, "ISOLATING (Manual)", reason=f"Player Action ({self.player_name})", related_log_type="SYS_ISOLATION_INITIATED"):
             # Make it immediate for simplicity after action.
    ...
    #        # Specific Agent Logic (Using NEW agent names and flags)
    #        # <<< NAME CHANGE >>> Paul Kahn -> Laura Mitchell
    #        if agent_name == "Laura Mitchell" and not agent_flags.get("called_by_player"):
    #            # Get delay using NEW name key
    #            laura_delay_base = BASE_AGENT_INITIATIVE_DELAY_SECONDS.get("Laura Mitchell", 1)
    #            laura_delay_sim_secs = laura_delay_base * effective_intensity_mod # Delay in sim seconds
    #            if not agent_flags.get("attempted_call") and time_since_start_seconds >= laura_delay_sim_secs:
    #                reason = f"Laura Mitchell's urgency timer ({laura_delay_sim_secs:.0f}s sim time)"; should_agent_call = True
    #        # <<< NAME CHANGE >>> Hao Wang -> James Bennett
    #        elif agent_name == "James Bennett" and agent_data.get("state") == "investigating":
    ...
    #                      reason = f"James Bennett's idle update ({update_interval_sim_secs:.0f}s sim)"; should_agent_call = True; is_update = True
    #             elif not last_comm and time_since_start_seconds > (update_interval_sim_secs / 2):
    #                  reason = f"James Bennett's initial update"; should_agent_call = True; is_update = True
    #        # <<< NAME CHANGE >>> Lynda Carney -> Ethan Kim
    #        elif agent_name == "Ethan Kim" and agent_data.get("state") == "busy_monitoring":
    ...
    #             # Use agent's specific flags
    #             if enc_sys and not agent_flags.get("alerted_encryption"):
    #                  reason = f"Ethan Kim alert: Encryption DETECTED on {list(enc_sys.keys())}!"; should_agent_call = True; is_update = True; agent_flags["alerted_encryption"] = True
    ...
    #                 reason = f"Ethan Kim alert: CRITICAL status on {list(set(alert_keys))}!"; should_agent_call = True; is_update = True; agent_flags["alerted_critical"] = True
    #             else:
    ...
    #                       last_comm_sim_utc = last_comm.astimezone(timezone.utc)
    #                       if (now_sim_utc - last_comm_sim_utc).total_seconds() >= update_interval_sim_secs:
    #                           reason = f"Ethan Kim's idle update ({update_interval_sim_secs:.0f}s sim)"; should_agent_call = True; is_update = True
    #                  elif not last_comm and time_since_start_seconds > (update_interval_sim_secs / 2):
    #                       reason = f"Ethan Kim's initial update"; should_agent_call = True; is_update = True
    ...
    #              condition_met = False
    #              try:
    #                  # Pass self (sim) and current sim time (now_sim_utc) to condition lambda
    #                  # Lambdas reference player actions via self._check_player_action
    #                  # Lambdas reference system status via self.system_status
    #                  condition_met = rule["condition"](self, now_sim_utc)
    #              except Exception as e:
    ...
    #                  try:
    #                      # Action lambda gets self (sim)
    #                      action_result = rule["action"](self)
    #                      # Assume True/None means changed, False means explicitly no change
    ...
    #                      break # Stop processing rules if one fails
    ...
    #                      if not self.metrics: self._reset_metrics()
    #                      # Ensure escalations_triggered is an int before incrementing
    #                      current_esc_metric = self.metrics.get("escalations_triggered", 0)
    #                      self.metrics["escalations_triggered"] = current_esc_metric + 1 if isinstance(current_esc_metric, int) else 1
    ...
    #                      # Check intensity immediately after escalation (uses sim time)
    #                      self.check_dynamic_intensity()
    #                      # Only trigger one escalation per check cycle
    #                      break
    #          # Update last check time if interval was met (regardless of escalation)
    ...
    def check_background_events(self):
        ...
        for agent_name, agent_data in self.agents.items():
           ...
           if last_contact and isinstance(last_contact, datetime) and last_contact.tzinfo:
               last_contact_sim_utc = last_contact.astimezone(timezone.utc)
               if (now_sim_utc - last_contact_sim_utc) < contact_cooldown:
                   # self.log_event(f"Debug: {agent_name} in cooldown.", level="debug") # Optional
                   continue # Still in cooldown (based on sim time)
           ...
           if agent_name == "Laura Mitchell" and not agent_flags.get("called_by_player"):
               # --- START: MODIFIED SECTION FOR LAURA MITCHELL ---
               # Check if we've already processed the special initial urgent call logic for Laura
               if not agent_flags.get("attempted_initial_urgent_call_logic"):
                   # Decide randomly if Laura makes an *extra* early, special call
                   # For example, a 75% chance she makes this special early call
                   if random.random() < 0.75: # Adjust probability as needed
                       # Define a very short delay specifically for this initial urgent call.
                       # This delay is independent of her normal BASE_AGENT_INITIATIVE_DELAY_SECONDS.
                       special_initial_delay_base_seconds = 1 # Make this very short, e.g., 1 simulation second
                       special_initial_delay_sim_secs = special_initial_delay_base_seconds * effective_intensity_mod

                       # Check if enough simulation time has passed for this special call
                       # AND she hasn't already "attempted_call" (e.g. from a previous failed special attempt in a very fast loop)
                       if time_since_start_seconds >= special_initial_delay_sim_secs and not agent_flags.get("attempted_call"):
                           reason = f"Laura Mitchell's *initial* urgent outreach ({special_initial_delay_sim_secs:.0f}s sim time)"
                           should_agent_call = True
                           # agent_flags["attempted_call"] = True # This flag will be set when she is actually picked to call or in handle_agent_contact
                           self.log_event(f"Debug: Laura Mitchell special initial call triggered. Reason: {reason}", level="debug")
                   # else:
                       # self.log_event(f"Debug: Laura Mitchell *did not* make the special random initial call this time.", level="debug")

                   # Mark that we've processed the logic for her special initial call attempt,
                   # regardless of whether she made the call or not.
                   # This prevents this random block from running repeatedly.
                   agent_flags["attempted_initial_urgent_call_logic"] = True
               # --- END: MODIFIED SECTION FOR LAURA MITCHELL ---

               # --- START: STANDARD INITIATIVE LOGIC FOR LAURA (Fallback or Regular) ---
               # This runs if:
               # 1. The special initial call logic has been processed (flag is set) AND she wasn't selected for that special call.
               # 2. OR if the special call was made, but now enough time has passed for her *regular* initiative.
               # We only proceed if 'should_agent_call' isn't already true from the special logic
               # and she hasn't already "attempted_call" in this simulation cycle (e.g. if special call triggered but she wasn't picked from agent_wants_to_call)
               if not should_agent_call and not agent_flags.get("attempted_call"):
                   laura_delay_base = BASE_AGENT_INITIATIVE_DELAY_SECONDS.get("Laura Mitchell", 1) # Her 'normal' configured delay (1 sec)
                   laura_delay_sim_secs = laura_delay_base * effective_intensity_mod

                   if time_since_start_seconds >= laura_delay_sim_secs:
                       reason = f"Laura Mitchell's standard urgency timer ({laura_delay_sim_secs:.0f}s sim time)"
                       should_agent_call = True
                       # self.log_event(f"Debug: Laura Mitchell standard call triggered. Reason: {reason}", level="debug")
               # --- END: STANDARD INITIATIVE LOGIC FOR LAURA ---
                ...
                if last_comm and isinstance(last_comm, datetime) and last_comm.tzinfo:
                     ...
                     if (now_sim_utc - last_comm_sim_utc).total_seconds() >= update_interval_sim_secs:
                         reason = f"James Bennett's idle update ({update_interval_sim_secs:.0f}s sim)"; should_agent_call = True; is_update = True
                elif not last_comm and time_since_start_seconds > (update_interval_sim_secs / 2): # Ensure some time has passed for a first update
                     reason = f"James Bennett's initial update"; should_agent_call = True; is_update = True

           elif agent_name == "Ethan Kim" and agent_data.get("state") == "busy_monitoring":
                status_copy = self.system_status.copy() if isinstance(self.system_status, dict) else {} # Ensure it's a dict
                crit_sys = {k:v for k,v in status_copy.items() if isinstance(v,str) and any(s in v for s in ["CRITICAL", "COMPROMISED"])}
                enc_sys = {k:v for k,v in status_copy.items() if isinstance(v,str) and "ENCRYPTING" in v}
                enc_crit_sys = {k:v for k,v in status_copy.items() if isinstance(v,str) and "ENCRYPTED (CRITICAL)" in v}
                ...
                if enc_sys and not agent_flags.get("alerted_encryption"):
                    ...
                    alert_keys = list(crit_sys.keys()) + list(enc_crit_sys.keys())
                    reason = f"Ethan Kim alert: CRITICAL status on {list(set(alert_keys))}!"; should_agent_call = True; is_update = True; agent_flags["alerted_critical"] = True
                else: # Standard update logic for Ethan if no critical alerts pending for him
                     update_interval_base = (BASE_IDLE_AGENT_UPDATE_INTERVAL_SECONDS / 1.5) # Ethan updates more frequently
                     update_interval_sim_secs = update_interval_base * effective_intensity_mod
                     last_comm = agent_data.get("last_update_time") or agent_data.get("last_contact_time")
                     if last_comm and isinstance(last_comm, datetime) and last_comm.tzinfo:
                          last_comm_sim_utc = last_comm.astimezone(timezone.utc)
                          if (now_sim_utc - last_comm_sim_utc).total_seconds() >= update_interval_sim_secs:
                              reason = f"Ethan Kim's idle update ({update_interval_sim_secs:.0f}s sim)"; should_agent_call = True; is_update = True
                     elif not last_comm and time_since_start_seconds > (update_interval_sim_secs / 3): # Ethan's first update can be quicker too
                          reason = f"Ethan Kim's initial update"; should_agent_call = True; is_update = True
           # --- End Specific Agent Logic ---

           # Save updated flags back immediately if they were changed for this agent
           # This is important for flags like 'attempted_initial_urgent_call_logic', 'alerted_encryption', etc.
           if agent_flags != agent_data.get("flags", {}): # Only save if changed
                self.agents[agent_name]["flags"] = agent_flags

           if should_agent_call:
                self.log_event(f"Agent Initiative Check: {agent_name} wants to call. Reason: {reason}", level="info")
                # Check the 'attempted_call' flag for this specific agent for THIS CYCLE
                # to prevent adding them multiple times if, for example, special logic and standard logic both trigger
                # However, the current structure with `if not should_agent_call` handles this.
                # The `attempted_call` flag is more for `handle_agent_contact` to use to record the attempt for cooldowns and LLM prompts.
                # agent_flags["attempted_call"] = True # This flag will be set in handle_agent_contact
                # self.agents[agent_name]["flags"] = agent_flags # Persist this immediate attempt flag (already handled by end-of-loop save)

                agent_wants_to_call.append({"name": agent_name, "is_update": is_update, "reason": reason})
        # --- End Agent Loop ---

        if agent_wants_to_call:
            # Prioritize more "urgent" or "critical" reasons if multiple agents want to call
            # Example: Laura's initial outreach might be considered high priority
            def call_priority(call_info):
                r = call_info.get("reason", "").lower()
                if "laura mitchell's *initial* urgent outreach" in r: return 0
                if "critical" in r or "alert" in r or "encryption detected" in r: return 1
                if "laura mitchell" in r: return 2 # Other Laura calls
                return 3 # Regular updates

            agent_wants_to_call.sort(key=call_priority)
            caller_info = agent_wants_to_call[0] # Pick the highest priority

            self.log_event(f"Triggering agent contact from background check: {caller_info['name']} (Reason: {caller_info['reason']})", level="debug")
            self.update_agent_state(caller_info["name"], "trying_to_call_cto") # Agent is now busy trying
            # handle_agent_contact will set the agent's "attempted_call" flag and "last_contact_time"
            self.handle_agent_contact(caller_info["name"], initiated_by="agent", is_update=caller_info["is_update"])

        # --- 2. System Escalation Check (Using Sim Time) ---
        # (Your existing escalation check logic remains unchanged here)
        # ... (keep this section exactly as you had it) ...
        escalated_this_tick = False
        interval_met = True
        effective_intensity_mod_esc = self.current_intensity_mod if isinstance(self.current_intensity_mod, (float, int)) and self.current_intensity_mod > 0 else 1.0
        if self.last_escalation_check_time and self.last_escalation_check_time.tzinfo:
             try:
                 last_check_sim_utc = self.last_escalation_check_time.astimezone(timezone.utc)
                 effective_interval_sim_secs = BASE_ESCALATION_CHECK_INTERVAL_SECONDS * effective_intensity_mod_esc
                 if (now_sim_utc - last_check_sim_utc).total_seconds() < effective_interval_sim_secs:
                     interval_met = False
             except (TypeError, ValueError):
                  print("Warning: Error calculating escalation check interval.")
             ...
             for rule_index, rule in enumerate(self.selected_scenario["escalation_rules"]):
                 ...
                 if condition_met:
                     ...
                     if status_changed:
                         ...
                         self.check_dynamic_intensity()
                         break
             if interval_met: # Update last check time if interval was met (regardless of escalation occurring)
                self.last_escalation_check_time = now_sim_utc
    ...
    def enter_decision_point_shutdown(self, player_forced=False):
        ...
        # <<< NAME CHANGE >>> Hao Wang -> James Bennett, Paul Kahn -> Laura Mitchell
        # Check flags using NEW agent names
        james_advised = self.agents.get("James Bennett", {}).get("flags", {}).get("has_advised_caution", False)
        laura_demanded = self.agents.get("Laura Mitchell", {}).get("flags", {}).get("has_demanded_shutdown", False)
        ...
        # Check flags using NEW agent flags
        elif (james_advised and laura_demanded): ready_for_decision = True; decision_reason = "Key inputs received (James Bennett & Laura Mitchell)"
        ...
        if ready_for_decision:
            ...
            # Update summary lines with NEW agent names
            summary_lines = [ f"Decision Point: Containment Strategy for '{self.selected_scenario_key}'", f"Reason: {decision_reason}",
                "--- Inputs ---",
                f"- James Bennett Advised Caution? {'YES' if james_advised else 'NO'}",
                f"- Laura Mitchell Urged Shutdown? {'YES' if laura_demanded else 'NO'}",
            ...
            # self.advance_time(1) # <<< REMOVED
             ...
             # Update missing prompt with NEW agent names
             missing = []
             if not james_advised: missing.append("James Bennett (Caution Advice)")
             if not laura_demanded: missing.append("Laura Mitchell (Shutdown Urgency)")
             ...
             self.emit_event("display_message", {"speaker": "System", "message": msg})
             # If somehow state was already Decision Point, reset it if not ready
    ...
    def _call_llm_for_rating(self) -> dict:
        ...
        try:
            # Uses NEW agent names within metrics implicitly
        ...
        rating_context = {
            ...
            "metrics": serializable_metrics, # Contains NEW agent names if relevant
            "event_highlights": list(self.event_log_history[-30:]) # Event logs might contain NEW names
        }
        ...
        try:
            ...
            # List will contain NEW agent names
        ...
        # Prompt uses the collected metrics, which implicitly contain new agent names if they were contacted
        ...
        try:
            ...
            try:
                # Try extracting JSON even if wrapped in text
                ...
                if json_match:
                    ...
                    # Try parsing directly if no clear markers
                ...
                # If still no valid JSON after attempts
            ...
            except Exception as parse_e: # Catch broader errors during parsing attempts
                raise json.JSONDecodeError(f"Unexpected parsing error: {parse_e}", raw_response, 0)
                    ...
                    if key not in rating_json: errors.append(f"Missing required key: '{key}'")
                        ...
                        if "score" in key:
                            if not isinstance(value, int) or not (1 <= value <= 10):
                                ...
                                # Attempt reasonable coercion or default
                                try: validated_rating[key] = max(1, min(10, int(value)))
                                except: validated_rating[key] = 5 # Default score on failure
            ...
            if errors:
                ...
                # Return error but include partial/raw rating if available
        ...
        except ValueError as e: # Catch errors raised during processing
    ...
    def trigger_debrief(self):
        ...
        try:
            ...
            if isinstance(t_crit, datetime) and t_crit.tzinfo and \
               self.simulation_start_time and isinstance(self.simulation_start_time, datetime) and self.simulation_start_time.tzinfo:
                ...
                except (TypeError, ValueError): pass # Ignore errors during calculation
            ...
            # Get the set/list of NEW agent names contacted
            ...
            if isinstance(key_actions_list, list):
                for action in key_actions_list:
                    # Action targets might be NEW agent names
            ...
            metrics_summary = [
                ...
                # Display NEW agent names contacted
        ...
        # --- Optional Analyst Briefing Prompt Removed ---
        # Now rating happens, then maybe an end screen or final step.
    ...
    def end_simulation(self):
        ...
        # Perhaps call cleanup here? Or let the calling context handle it.
        # self.cleanup()
    ...
    def get_status_summary(self, compact=True):
        ...
        if compact:
            ...
            # Define severity order for sorting non-nominal statuses
            severity_order = {"CRITICAL": 0, "HIGH": 1, "WARN": 2, "INFO": 3, "LOW": 4, "MEDIUM": 5} # Added MEDIUM
            def sort_key(item):
                key, value = item
                # Handle statuses like "ISOLATED (Manual)" -> take "ISOLATED"
                sev_word = value.split(" ")[0]
                # Map internal status keyword to severity level string (INFO, WARN, etc.)
                sev_level_str = LOG_SEVERITY.get(sev_word, "INFO") # Default to INFO if not found
                # Get the numeric score for sorting
                sev_score = severity_order.get(sev_level_str, 99) # Default to low priority
                return (sev_score, key) # Sort by severity score, then alphabetically by key

            sorted_items = sorted(non_nominal.items(), key=sort_key)
            return ", ".join([f"{k}: {v}" for k, v in sorted_items])
        else:
            # For detailed view, just sort alphabetically by system key
    ...
    def cleanup(self):
        ...
        self._clear_current_events()
        # Potentially release resources here if any were held (e.g., close files, connections)
        # In this case, primarily about ensuring state is final.
